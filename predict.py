...
def get_argparser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="gen_seg",
        choices=["voc", "cityscapes", "gen_seg"],
    ...
    with torch.no_grad():
        ...
        for img_path in tqdm(image_files):
            ...
            camera_angle = np.array(
                Image.open(f"ignore_masks/{img_name.split('_')[0]}.png")
            )
            ...
            tmp_preds = np.array(
                T.Resize(camera_angle.shape, interpolation=T.InterpolationMode.NEAREST)(
                    Image.fromarray(colorized_preds)
                )
            )
